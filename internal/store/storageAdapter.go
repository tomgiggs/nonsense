	...
	if Storage == nil {
		if len(conf.Storage.Name) > 0 {
			if ad, ok := AvailableAdapters[conf.Storage.Name]; ok {
				...
				switch conf.Storage.Name {
				case "mysql":
					Storage = NewMysqlAdapter()
				case "mongo":
					panic("mongodb adapter is not available")
				case "cassandra":
					panic("cassandra adapter is not available")
				default:
					Storage = NewMysqlAdapter()
				}
			}
		} else {
			return fmt.Errorf("store: db adapter is not specified. Please set storage.name in config file")
		}
	}
	Storage.Open(conf)
	return nil
